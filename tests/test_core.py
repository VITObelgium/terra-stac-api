...
async def test_get_protected_collection_unauthorized(client):
    response = await client.get(str(ENDPOINT_COLLECTIONS / COLLECTION_PROTECTED), auth=MockAuth("unsufficient"))
    assert response.status_code == codes.FORBIDDEN


async def test_get_protected_collection_authorized(client):
    response = await client.get(str(ENDPOINT_COLLECTIONS / COLLECTION_PROTECTED), auth=MockAuth(ROLE_PROTECTED))
    assert response.status_code == codes.OK


async def test_get_protected_collection_admin(client):
    response = await client.get(str(ENDPOINT_COLLECTIONS / COLLECTION_PROTECTED), auth=MockAuth(ROLE_ADMIN))
    assert response.status_code == codes.OK


async def test_get_collection_items(client):
    response = await client.get(str(ENDPOINT_COLLECTIONS / COLLECTION_S2_TOC_V2 / "items"))
    assert response.status_code == codes.OK
    assert len(response.json()['features']) == 4


async def test_get_protected_collection_items_unauthorized(client):
    response = await client.get(str(ENDPOINT_COLLECTIONS / COLLECTION_PROTECTED / "items"))
    assert response.status_code == codes.UNAUTHORIZED
    assert "features" not in response.json()


async def test_get_protected_collection_items_authorized(client):
    response = await client.get(str(ENDPOINT_COLLECTIONS / COLLECTION_PROTECTED / "items"), auth=MockAuth(ROLE_PROTECTED))
    assert response.status_code == codes.OK
...
async def test_search_protected(client):
    response = await client.get(str(ENDPOINT_SEARCH), params={"collections": f"{COLLECTION_S2_TOC_V2},{COLLECTION_PROTECTED}", "limit": 100})
    assert response.status_code == codes.UNAUTHORIZED
    assert "features" not in response.json()


async def test_search_protected_authorized(client):
    response = await client.post(str(ENDPOINT_SEARCH), json={"collections": [COLLECTION_S2_TOC_V2, COLLECTION_PROTECTED], "limit": 100}, auth=MockAuth(ROLE_PROTECTED))
    assert response.status_code == codes.OK
    rj = response.json()
