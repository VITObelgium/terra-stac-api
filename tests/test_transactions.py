...
async def test_create_collection_authenticated_no_editor(client, extra_collection):
    response = await client.post(str(ENDPOINT_COLLECTIONS), json=extra_collection, auth=MockAuth(ROLE_PROTECTED))
    assert response.status_code == codes.FORBIDDEN

async def test_create_collection_authorized(client, extra_collection):
    auth = MockAuth(ROLE_PROTECTED, ROLE_EDITOR)
    response = await client.post(str(ENDPOINT_COLLECTIONS), json=extra_collection, auth=auth)
    assert response.status_code == codes.OK

    response = await client.get(str(ENDPOINT_COLLECTIONS / extra_collection["id"]), auth=auth)
    assert response.status_code == codes.OK
...
async def test_update_collection_no_write_permission(client, collections):
    protected = collections[COLLECTION_PROTECTED]
    response = await client.put(str(ENDPOINT_COLLECTIONS), json=protected, auth=MockAuth(ROLE_PROTECTED))
    assert response.status_code == codes.FORBIDDEN
...
async def test_update_collection_authorized(client, collections):
    ...
    response = await client.get(str(ENDPOINT_COLLECTIONS / COLLECTION_PROTECTED), auth=auth)
    assert response.status_code == codes.OK
...
async def test_delete_collection_unauthorized(client):
    response = await client.delete(str(ENDPOINT_COLLECTIONS / COLLECTION_S2_TOC_V2), auth=MockAuth("unsufficient"))
    assert response.status_code == codes.FORBIDDEN

async def test_delete_collection_authorized(client, api):
    response = await client.delete(str(ENDPOINT_COLLECTIONS / COLLECTION_S2_TOC_V2), auth=MockAuth(ROLE_SENTINEL2))
    assert response.status_code == codes.NO_CONTENT
    await api.client.database._refresh()
    
    response = await client.get(str(ENDPOINT_COLLECTIONS))
    assert response.status_code == codes.OK
    assert COLLECTION_S2_TOC_V2 not in list(c['id'] for c in response.json()["collections"])